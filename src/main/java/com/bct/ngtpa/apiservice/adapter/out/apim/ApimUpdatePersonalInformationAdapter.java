...
public class ApimUpdatePersonalInformationAdapter implements ApimUpdatePersonalInformationPort {
    ...
    public Mono<UpdatePersonalInformationResult> updateMemberInfo(UpdateMemberInfoCommand command) {
        ...
        return apimCertificateService.getBctPublicKey()
                .flatMap(publicKey -> {
                    var request = apimPayloadCryptoService.encryptRequest(
                            API_NAME, toApimRequest(command), UpdateMemberInfoApimRequest.class, publicKey);
                    return apimWebClientFacade.post(API_NAME, request)
                            .map(body -> apimPayloadCryptoService.decryptResponseEnvelope(
                                    API_NAME, body, UpdateMemberInfoApimDataItem.class, publicKey))
                            .map(this::toResult);
                })
                .onErrorMap(ApimCryptoException.class, ex ->
                        new ApimException(HttpStatus.INTERNAL_SERVER_ERROR,
    ...
    private UpdatePersonalInformationResult toResult(ApimResponseEnvelope<UpdateMemberInfoApimDataItem> response) {
        ...
        if (CollectionUtils.isEmpty(dataItems) || dataItems.getFirst() == null || !dataItems.getFirst().isSuccess()) {
            throw new ApimException(HttpStatus.BAD_GATEWAY, ErrorCodes.APIM_RESPONSE_INVALID,
                    "APIM personal information update was not successful.");
        }
        return new UpdatePersonalInformationResult(true);
    }
}
