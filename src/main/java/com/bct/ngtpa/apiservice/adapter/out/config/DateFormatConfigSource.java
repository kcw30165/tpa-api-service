...
public class DateFormatConfigSource implements ConfigSource {

    private static final String DEFAULT_LANGUAGE = Locale.ENGLISH.toString();
    private static final String CODE = "date";
    ...
    public ConfigCategory category() {
        return ConfigCategory.DISPLAY_DATE_FORMAT;
    }
    ...
    private Optional<String> resolveForLanguage(String key, String language) {
        var formats = dateFormatProperties.getLocaleFormats(language);
        if (formats.isEmpty()) {
            return Optional.empty();
        }
