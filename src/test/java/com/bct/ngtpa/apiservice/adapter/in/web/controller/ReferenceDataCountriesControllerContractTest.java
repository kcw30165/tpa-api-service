...
class ReferenceDataCountriesControllerContractTest {
    ...
    @Test
    void countriesEndpointUsesLanguageOnlyBoundary() {
        ...
        GetReferenceDataCountriesUseCase useCase = command -> {
            ...
            return Mono.just(new ReferenceDataCountriesResult(
                    List.of(new ReferenceDataOption("HK", "Hong Kong")),
                    List.of(new ReferenceDataOption("+852", "Hong Kong"))));
        };

        var body = new ReferenceDataController(useCase)
                .getCountries("en")
        ...
        assertEquals("HK", body.countries().get(0).value());
        assertEquals("+852", body.callingCodes().get(0).value());
    }
}
